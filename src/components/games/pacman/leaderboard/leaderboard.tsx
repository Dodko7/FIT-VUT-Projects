...
/**
 * @brief Renders the leaderboard table with levels
 * @param {Props} props - Component props containing optional initial scores
...
export default function LeaderboardTable({ initialScores }: Props) {
    ...
    return (
        <div className="max-w-md mx-auto p-10 bg-gray-900 rounded-lg border-2 border-blue-900">
            <h1 className="text-center font-['Press_Start_2P'] text-2xl text-yellow-400 mb-10">
                PACMAN Leaderboard
            </h1>
            {/* Leaderboard level picker */}    
                ...
                {LEVELS.map((level) => (
                    <button
                        ...
                        className={`px-3 py-2 font-['Press_Start_2P'] text-s border-b-4 transition-all  
                            ${activeLevel === level.id ? `${level} text-yellow-400` : "text-gray-500 border-transparent hover:text-yellow-400"}`}
                    >
                        {level.label}
                    </button>
                ))}
