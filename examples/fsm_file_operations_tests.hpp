...
TestFramework::TestSuite createTestSuite() {
    ...
    suite.addTest("Save and load FSM with transitions", []() {
        ...
        nlohmann::json transitions1 = loadManager.getStateTransitions("State1");
        nlohmann::json transitions2 = loadManager.getStateTransitions("State2");
        
        bool hasTransitions1 = !transitions1.empty();
        bool hasTransitions2 = !transitions2.empty();
        
        // Clean up
        cleanupTestFile(testFilePath);
        
        return TestFramework::assert_that(hasTransitions1 && hasTransitions2, 
                                        "Loaded FSM should have the same transitions as saved FSM");
    });
    ...
    suite.addTest("Save to a potentially restricted location", []() {
        ...
        return TestFramework::TestResult(true, "Save to restricted location " + 
                                    std::string(saved ? "succeeded (unexpected)" : "failed (expected)"), 
                                    std::chrono::milliseconds(0));
    });
