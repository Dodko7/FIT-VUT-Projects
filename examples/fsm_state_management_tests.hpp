...
TestFramework::TestSuite createTestSuite() {
    ...
    suite.addTest("Remove a non-existent state", []() {
        ...
        bool removed = manager.removeState("NonExistentState");
        
        return TestFramework::assert_that(!removed, "Removing a non-existent state should fail");
    });
    ...
    suite.addTest("Update state", []() {
        ...
        bool updated = manager.updateState("State1", "UpdatedState", "console.log('Updated');", 'U', 
        ...
        for (const auto& name : stateNames) {
            if (name == "UpdatedState") {
                nameUpdated = true;
                break;
            }
        }
        
        nlohmann::json stateInfo = manager.getStateInfo("UpdatedState");
        bool isFinal = stateInfo["isFinal"].get<bool>();
        char output = stateInfo["output"].get<std::string>()[0];
        
        return TestFramework::assert_that(nameUpdated && isFinal && output == 'U', 
                                         "State should be updated successfully with all attributes changed");
    });
